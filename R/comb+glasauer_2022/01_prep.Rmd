---
title: "01 Prep - Combined Organelles and Glasauer et al. 2022"
author:
...
if (file.exists(rds)) {
  ...
  seurat1 <- readRDS(file.path(cache_dir, "..", "..", "comb", "02", "rca_seurat.rds"))
  seurat2 <- readRDS(file.path(cache_dir, "..", "..", "glasauer_2022", "02", "rca_seurat.rds"))

  genes <- Reduce(intersect, list(rownames(seurat1), rownames(seurat2)))
  seurat1 <- seurat1[genes, ]
  seurat2 <- seurat2[genes, ]

  seurat <- merge(seurat1, seurat2)
  remove(seurat1, seurat2)
...
if (file.exists(rds)) {
  ...
  seurat <- RunUMAP(seurat, 1:30, min.dist = 0.75, verbose = FALSE)
  ...
  seurat$timepoint_merged <- sub("6m", "m6", seurat$timepoint)
  seurat$timepoint_merged <- sub("4m", "m4", seurat$timepoint_merged)
  seurat$timepoint_merged <- sub("2m", "m2", seurat$timepoint_merged)
  seurat$timepoint_merged <- sub("D120", "m4", seurat$timepoint_merged)
  seurat$timepoint_merged <- sub("D72", "m2", seurat$timepoint_merged)

  seurat$genotype_merged <- sub("406RR", "WT", seurat$genotype)
  seurat$genotype_merged <- sub("406RW", "HET", seurat$genotype_merged)
  seurat$genotype_merged <- sub("406WW", "HOM", seurat$genotype_merged)

  saveRDS(seurat, rds, compress = FALSE)
}

ElbowPlot(seurat, 50) + geom_vline(xintercept = 30, color = "red")
