---
title: "08 GSE DEG Overlap - Combined Organelles"
author:
...
knit: (function(inputFile, encoding) {
  rmarkdown::render(inputFile, encoding = encoding, output_file = file.path(
    "..", "..", "results", unlist(strsplit(getwd(), "/"))[8], "08_GSE_DEG_overlap.html"
  ))})
---
...
packages <- c("conflicted", "GSEABase", "biomaRt", "DT")
...
```{r}
percentage_df <- data.frame(matrix(nrow = 0, ncol = 2))
  ...
  if (nrow(GSE) > 0) {
    for (j in seq(nrow(GSE))) {
      ...
      if (any(DEG$X %in% current_gene_set[[1]])) {
        ...
        add_df <- data.frame(x = names(current_gene_set), y = percentage)
...
```{r}
percentage_df <- percentage_df[order(percentage_df$y, decreasing = TRUE), ]
colnames(percentage_df) <- c("Gene Set", "Percentage DEG")
