...
if (nrow(all) > 25) {
  ...
  mat <- logcounts(sce)[which(rownames(sce) %in% rownames(all)), ]  
}
...
if (nrow(all) > 25) {
  ...
  DotPlot(
    seurat_list[[seurat_idx]], features = rownames(all), cols = c("blue", "red")
  ) + RotatedAxis()
  VlnPlot(seurat_list[[seurat_idx]], rownames(all))
}
```
...
if (nrow(seurat_DEGs) > 25) {
  VlnPlot(seurat_list[[seurat_idx]], rownames(seurat_DEGs)[1:12])
} else {
  VlnPlot(seurat_list[[seurat_idx]], rownames(seurat_DEGs))
}
