...
packages <- c(
  ...
  "data.table", "GSVA", "edgeR", "ComplexHeatmap", "patchwork"
)
...
if (file.exists(rds)) {
  ...
  if (file.exists(rds)) {
    ...
    rca <- dataProject(
      rca, "Custom",
      file.path(cache_dir, "bhaduri_primary_panel_Type.rds"), "spearman",
      min.cell.number.expressing = 0
    )
    rca <- computeUMAP(rca)
    rca <- estimateCellTypeFromProjection(rca, cSCompute = TRUE)
    rca <- dataSClust(rca, nPCs = 7, corMeth = "spearman")
...
seurat$rca_names <- factor(unlist(rca$cell.Type.Estimate))
seurat$rca_clusters <- rca$clustering.out$cellTree
...
red_dim_plot(seurat, "rca_umap1", "rca_umap2", "rca_names", "cat")
...
red_dim_plot(seurat, "rca_umap1", "rca_umap2", "genotype", "cat")
...
rca_MAD <- vector("list", length(unique(seurat$rca_names)))
names(rca_MAD) <- unique(seurat$rca_names)
for (i in seq_along(rca_MAD)) {
  cells <- names(seurat$rca_names[seurat$rca_names == names(rca_MAD)[i]])
...
seurat$rca_names_pruned <- factor(
  replace(as.character(seurat$rca_names), rename, "Unclassifed")
)
