...
### Primary Broad Clusters Merged Small Groups

```{r}
rds <- file.path(cache_dir, "bhaduri_primary_rca_Type_merged.rds")
rds2 <- file.path(cache_dir, "bhaduri_primary_pseudobulk_Type_merged.rds")
if (file.exists(rds)) {
  rca <- readRDS(rds)
  ref <- readRDS(rds2)
} else {
  ...
  ref$Type[which(ref$Type == "Mural" | ref$Type == "Endothelial")] <- "Vascular"
  ref$Type[which(ref$Type == "OPC" | ref$Type == "Microglia")] <- "Radial Glia"
  ref$Type[which(ref$Type == "Excitatory Neuron ")] <- "Excitatory Neuron"

  rds <- file.path(cache_dir, "bhaduri_primary_pseudobulk_Type_merged.rds")
  if (file.exists(rds)) {
    ref <- readRDS(rds)
  } else {
    top <- nrow(ref) / 2
  ...
  rds <- file.path(cache_dir, "bhaduri_primary_panel_Type_merged.rds")
  if (file.exists(rds)) {
    ref <- readRDS(rds)
  } else {
    ref <- ref[-(which(rowSums(counts(ref)) == 0)), ]
  ...
  if (file.exists(rds)) {
    ...
    rca <- createRCAObject(GetAssayData(seurat, "counts"), GetAssayData(seurat))
    rca <- dataProject(
      rca, "Custom", file.path(cache_dir, "bhaduri_primary_panel_Type_merged.rds"), "spearman"
    )
    rca <- computeUMAP(rca)
    rca <- estimateCellTypeFromProjection(rca)
    saveRDS(rca, rds)
