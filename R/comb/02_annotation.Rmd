...
### Organoid Broad Clusters

```{r}
rds <- file.path(cache_dir, "bhaduri_organoid_rca_Type.rds")
rds2 <- file.path(cache_dir, "bhaduri_organoid_pseudobulk_Type.rds")
if (file.exists(rds)) {
  rca <- readRDS(rds)
  ref <- readRDS(rds2)
} else {

  rds <- file.path(cache_dir, "bhaduri_organoid_ref.rds")
  if (file.exists(rds)) {
    ref <- readRDS(rds)
  } else {
    ref <- fread(
      file.path(data_dir, "bhaduri_2020", "organoid", "organoidsmatrix_nonorm.txt"),
      data.table = FALSE
    )
    gc()
    rownames(ref) <- ref[ , 1]
    ...
    meta <- read.delim(file.path(data_dir, "bhaduri_2020", "organoid", "meta.tsv"), row.names = 1)
    meta <- meta[-(which(meta$Type == "Outlier")), ]
    meta <- meta[-(which(meta$Type == "Unknown")), ]
    meta <- meta[-(which(meta$Subtype == "lowquality")), ]
  
    barcode <- sub(".*_", "", rownames(meta))
    rownames(meta) <- paste(barcode, meta$Sample, sep = " ")
    ref <- ref[ , which(colnames(ref) %in% rownames(meta))]
    meta <- meta[rownames(meta) %in% colnames(ref), ]
    meta <- meta[match(colnames(ref), rownames(meta)), ]
  ...
  rds <- file.path(cache_dir, "bhaduri_organoid_pseudobulk_Type.rds")
  if (file.exists(rds)) {
    ref <- readRDS(rds)
  } else {
    top <- nrow(ref) / 2
  ...
  rds <- file.path(cache_dir, "bhaduri_organoid_panel_Type.rds")
  if (file.exists(rds)) {
    ref <- readRDS(rds)
  } else {
    ref <- ref[-(which(rowSums(counts(ref)) == 0)), ]
  ...
  if (file.exists(rds)) {
    ...
    rca <- createRCAObject(GetAssayData(seurat, "counts"), GetAssayData(seurat))
    rca <- dataProject(
      rca, "Custom", file.path(cache_dir, "bhaduri_organoid_panel_Type.rds"), "spearman"
    )
    rca <- computeUMAP(rca)
    rca <- estimateCellTypeFromProjection(rca)
    saveRDS(rca, rds)
...
### Organoid Sub Clusters

```{r}
rds <- file.path(cache_dir, "bhaduri_organoid_rca_Subtype.rds")
rds2 <- file.path(cache_dir, "bhaduri_organoid_pseudobulk_Subtype.rds")
if (file.exists(rds)) {
  rca <- readRDS(rds)
  ref <- readRDS(rds2)
} else {
  ...
  rds <- file.path(cache_dir, "bhaduri_organoid_pseudobulk_Subtype.rds")
  if (file.exists(rds)) {
    ref <- readRDS(rds)
  } else {
    top <- nrow(ref) / 2
  ...
  rds <- file.path(cache_dir, "bhaduri_organoid_panel_Subtype.rds")
  if (file.exists(rds)) {
    ref <- readRDS(rds)
  } else {
    ref <- ref[-(which(rowSums(counts(ref)) == 0)), ]
  ...
  if (file.exists(rds)) {
    ...
    rca <- createRCAObject(GetAssayData(seurat, "counts"), GetAssayData(seurat))
    rca <- dataProject(
      rca, "Custom", file.path(cache_dir, "bhaduri_organoid_panel_Subtype.rds"), "spearman"
    )
    rca <- computeUMAP(rca)
    rca <- estimateCellTypeFromProjection(rca)
    saveRDS(rca, rds)
...
```{r}
rds <- file.path(cache_dir, "polioudakis_rca_Cluster.rds")
rds2 <- file.path(cache_dir, "polioudakis_pseudobulk_Cluster.rds")
if (file.exists(rds)) {
  rca <- readRDS(rds)
  ref <- readRDS(rds2)
} else {

  rds <- file.path(cache_dir, "polioudakis_ref.rds")
  if (file.exists(rds)) {
    ref <- readRDS(rds)
  } else {
    ...
    ref <- SingleCellExperiment(list(counts = raw_counts_mat), colData = meta)
    gc()
    saveRDS(ref, rds)
  }
  
  rds <- file.path(cache_dir, "polioudakis_pseudobulk_Cluster.rds")
  if (file.exists(rds)) {
    ref <- readRDS(rds)
  } else {
    top <- nrow(ref) / 2
    top <- ref[1:top, ]
    top <- aggregateAcrossCells(top, top$Cluster, "mean")
  
    bottom <- (nrow(ref) / 2) + 1
    bottom <- ref[bottom:(nrow(ref)), ]
    bottom <- aggregateAcrossCells(bottom, bottom$Cluster, "mean")
  ...
  rds <- file.path(cache_dir, "polioudakis_panel_Cluster.rds")
  if (file.exists(rds)) {
    ref <- readRDS(rds)
  } else {
    ref <- ref[-(which(rowSums(counts(ref)) == 0)), ]
  ...
  if (file.exists(rds)) {
    ...
    rca <- createRCAObject(GetAssayData(seurat, "counts"), GetAssayData(seurat))
    rca <- dataProject(
      rca, method = "Custom", file.path(cache_dir, "polioudakis_panel_Cluster.rds"), "spearman"
    )
    rca <- computeUMAP(rca)
    rca <- estimateCellTypeFromProjection(rca)
    saveRDS(rca, rds)
