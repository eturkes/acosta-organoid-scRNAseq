...
```{r}
sub_seurat <- seurat[ , which(seurat$rca_clusters_pruned == "Excitatory Neuron ")]
...
rds <- file.path(cache_dir, paste(sub_name, "bhaduri_primary_rca_Subtype.rds", sep = "_"))
if (file.exists(rds)) {
  rca <- readRDS(rds)
} else {
  rca <- createRCAObject(GetAssayData(file_list$comb, "counts"), GetAssayData(file_list$comb))
  rca <- dataProject(
    rca,
    "Custom",
    file.path(cache_dir, "..", "02", "bhaduri_primary_panel_Subtype.rds"),
...
file_list$comb$rca_subclusters <- factor(unlist(rca$cell.Type.Estimate))
red_dim_plot(file_list$comb, "umap1", "umap2", "rca_subclusters", "cat")
```
...
```{r}
sub_seurat <- seurat[ , which(seurat$rca_clusters_pruned == "Inhibitory Neuron")]
...
```{r}
sub_seurat <- seurat[ , which(seurat$rca_clusters_pruned == "IPC")]
...
```{r}
sub_seurat <- seurat[ , which(seurat$rca_clusters_pruned == "Mural")]
...
```{r}
sub_seurat <- seurat[ , which(seurat$rca_clusters_pruned == "Radial Glia")]
...
```{r}
sub_seurat <- seurat[ , which(seurat$rca_clusters_pruned == "Unclassified")]
