...
names(metadata_to_plot) <- c("sample", "timepoint", "seurat_clusters", "Phase")
# -------------------------------------------------------------------------
...
if (file.exists(rds)) {
  ...
  genes <- Reduce(
    ...
    list(
      ...
      rownames(files[[9]]), rownames(files[[10]]), rownames(files[[11]])
    )
  )
  for (i in seq_along(files)) {
  ...
  seurat <- merge(
    files[[1]],
    c(
      files[[2]], files[[3]], files[[4]], files[[5]], files[[6]],
      files[[7]], files[[8]], files[[9]], files[[10]], files[[11]]
    )
  )
  seurat$sample <- c(
